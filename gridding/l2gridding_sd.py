...
class var_grid:
	def __init__(self, map_bounds, res, vnames, dn, filter_val):
	...
	def add_file(self, filename):
		...
		tempfilter = np.array(ncf['dt1000']) > self.filter_val
		...
		for vname in self.vnames:
			...
			if vname == 'nh3_adj':
				...
				nh3_adj = np.array(ncf['nh3']) - scale*np.array(ncf['tpw']) - offset # IASI L2 night sea
		...
		if len(latlist) > 0:
			...
			# SD = math.sqrt( (1/N)* np.sum( np.square(vars - mean)))
			...
			for v in range(len(self.vnames)):
				...
				#self.components[v][dlatlist,dlonlist] += np.square(varlist[v] - self.means[v][dlatlist,dlonlist])
				...
				for i in range(len(dlatlist)):
					...
					while not np.isnan(self.grid_squares[v][ilat, jlon, index_next]) and index_next < len(self.grid_squares[v][ilat, jlon])-1:
						index_next += 1
					self.grid_squares[v][ilat,jlon, index_next] = varlist[v][i] # Mapping
...
options, operands = getopt(sys.argv[1:], "", ["year=","month=","dn=","instrt=","version=","subversion=","filter_val="])
...
if CLOBBER or not os.path.isfile(fpath+fname):
	...
	res = 0.25
	nh3_map = var_grid(map_bounds, res, ['nh3','tpw_base','tpw','dt1000', 'nh3_adj'], dn, filter_val)
