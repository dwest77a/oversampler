...
class l3_file: # All files going into a single end file
	def __init__(self, date, instrt, version, subversion, time, variables, longs, units):
		...
		try:
			...
			self.is_sd = False
			print('SDError: Standard Dev. Filter failed - Reference file not found - SD Filter inactive')
	...
	def oversample_pixel(self, pixel_lat, pixel_lon, pixel_snum, values, errors, pixel_theta):
		...
		# Update for GOME wide-intermediate-narrow swath pixel sizes ### Add to ostool.py
		# Scan line is ACROSS-track position
		...
		is_complete = False
		square_count = 0
		# Square count above/below pixel (lat)
		...
		while not is_complete:
			...
			if data_lat_check < len(self.calcdatae[0][0]):
				for dl in [[-1,0],[1,1]]:
					...
					is_zero = False
					side_count = dl[1]
					# delta lon (dl) parts [-1, 0] and [1,1]
					# where 0th part is starting point for side count (0-left, 1-right so 0 is not double counted)
	...
	def save_to_nc(self):
		...
		if np.nanmax(self.calcdatae[0]) - np.nanmin(self.calcdatae[0]) != 0.0:
			print('Writting to ',out_dir+base_file_format)
