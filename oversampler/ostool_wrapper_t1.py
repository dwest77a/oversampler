...
mystring += '#SBATCH --job-name=os_main_{}\n'
...
# Current versions of base filtered data for cris/iasi
if instrt == 'cris':
	year_init = 2015
	year_fin = 2020
	version = 'v00.05'
...
def record_subversion(version): # Subversion
	...
	while not isvalid:
		...
		if new_version not in versions:
			...
			if clob == 'y':
				# Overwrite existing files with new description
				isvalid = True
			elif clob == 'M':
				# Overwrite/continue existing processing so no new description or entry
				isvalid = True
				skipdesc = True
	
	if new_version != 'replace' and skipdesc == False:
...
for year in range(year_init, year_fin+1):
	for month in range(month_init, month_fin+1):
		if operands[0] == 'M' or operands[0] == 'STM':
			...
			ym = str(year) + mth
			mthstring = mystring.format(str(ym))
			# Do batch processing for final component
			if operands[0] == 'M':
				tf = 'Monthly'
				mthstring += '#SBATCH --time=10:00\n'
				mthstring += '#SBATCH --time-min=2:00\n'
				mthstring += '#SBATCH --mem=2G\n'
			else:
				tf = 'STM'
				mthstring += '#SBATCH --time=120:00\n'
				mthstring += '#SBATCH --time-min=80:00\n'
				mthstring += '#SBATCH --mem=6G\n'
			if BATCH:
				day = 1
				# Assemble sbatch file contents
				ymd = str(year) + mth + format(day,'02d') + 'd'
				
				monthstring = mthstring
				monthstring += 'module add jaspy\n'
				monthstring += 'python ostool_t1.py '+ymd+' {} {} {} {}\n'.format(instrt, version, subversion, tf)
				...
				ymd = str(year) + mth + '01d'
				print(ymd)
				os.system('python ostool_t1.py '+ymd+' {} {} {} {}'.format(instrt, version, subversion, tf))
		else:
			# Do batch processing of daily components
			mth = format(month, '02d')
			for day in range(1,days_in_month[month-1]+1):
				daystring = mystring.format(str(year) + mth + format(day,'02d'))
				# Assemble sbatch file contents
				ymd = str(year) + mth + format(day,'02d') + 'd'
				daystring += '#SBATCH --time=10:00\n'
				daystring += '#SBATCH --time-min=2:00\n'
				daystring += '#SBATCH --mem=2G\n'
				monthstring = daystring
				monthstring += 'module add jaspy\n'
				monthstring += 'python ostool_t1.py '+ymd+' {} {} {} Daily\n'.format(instrt, version, subversion)
				os.system('touch jbs_sbatch/jbs'+ymd+'.sbatch')
				f = open('jbs_sbatch/jbs'+ymd+'.sbatch','w')
				f.write(monthstring)
				f.close()
